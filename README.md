...
### 6.2 Producer 측 — `OutboxPublisherService`
...
### 6.3 Relay — `apps/workers/src/outbox-relay`
...
### 6.4 At-least-once 경계 — 의식적 trade-off
...
### 6.5 클라이언트용 — `Idempotency-Key` 헤더 (선택, 강력 권장)
...
### 6.9 Liveness vs Readiness (ADR-0001)
...
### 6.10 At-least-once audit / candle (ADR-0006)
...
### 6.11 Worker bulkhead 모드 (ADR-0005)
...
### 6.12 자동화 검증 스크립트 — `pnpm reliability:smoke` / `:chaos`
...
PASS 13/13   pnpm reliability:chaos
```

브로커가 30초 down → 5개 주문 누적 → 복구 후 60초 내 모두 drain → **0건 손실**.

### 6.13 대시보드 — `ORBIT — Reliability`
